class Ball:
    ...
    def move(self): 
        ...
        while(self.counter): # a while cycle for checking each move individually - this makes sure that the ball doesnt glitch out at high speed
            if self.y+self.stepy > 60 and self.y+self.stepy < height-60: # if not out of upper and lower boundaries, move
                self.y += self.stepy # move
                self.x += self.stepx
            else: # if touching the boundary, flip the y direction
